...
function render(level, canvas, camera) {
	...
	if(gameState > 0) {
		...
		for(var i=0; i<drawnLines; i++) {
			context.fillText(menuText[menuState][i].replace('!!!!!!!!!!!!!', topScores[0][0] + ': ' + topScores[0][1]).replace('@@@@@@@@@@@@@', topScores[1][0] + ': ' + topScores[1][1]).replace('#############', topScores[2][0] + ': ' + topScores[2][1]).replace('$$$$$$$$$$$$$', topScores[3][0] + ': ' + topScores[3][1]).replace('%%%%%%%%%%%%%', topScores[4][0] + ': ' + topScores[4][1]).replace('^^^^^^^^^^^^^', score).replace('&&&&&&&&&&&&&', (rank+1).toString()).replace('*************', ((name.length == 0) ? '_' : name)), Math.min(canvas.width-terminalImg.width, canvas.width/2.15) + terminalImg.width/5 + moved, canvas.height/7.5 + terminalImg.height/4 + 32*i);
		}
		context.fillText(menuText[menuState][drawnLines].replace('!!!!!!!!!!!!!', topScores[0][0] + ': ' + topScores[0][1]).replace('@@@@@@@@@@@@@', topScores[1][0] + ': ' + topScores[1][1]).replace('#############', topScores[2][0] + ': ' + topScores[2][1]).replace('$$$$$$$$$$$$$', topScores[3][0] + ': ' + topScores[3][1]).replace('%%%%%%%%%%%%%', topScores[4][0] + ': ' + topScores[4][1]).replace('^^^^^^^^^^^^^', score).replace('&&&&&&&&&&&&&', (rank+1).toString()).replace('*************', ((name.length == 0) ? '_' : name)).substring(0, drawnLetters) + ((menuText[menuState][drawnLines].replace('!!!!!!!!!!!!!', topScores[0][0] + ': ' + topScores[0][1]).replace('@@@@@@@@@@@@@', topScores[1][0] + ': ' + topScores[1][1]).replace('#############', topScores[2][0] + ': ' + topScores[2][1]).replace('$$$$$$$$$$$$$', topScores[3][0] + ': ' + topScores[3][1]).replace('%%%%%%%%%%%%%', topScores[4][0] + ': ' + topScores[4][1]).replace('^^^^^^^^^^^^^', score).replace('&&&&&&&&&&&&&', (rank+1).toString()).replace('*************', ((name.length == 0) ? '_' : name)).length > drawnLetters && gameState < -65) ? '_' : ''), Math.min(canvas.width-terminalImg.width, canvas.width/2.15) + terminalImg.width/5 + moved, canvas.height/7.5 + terminalImg.height/4 + 32*drawnLines);
		...
		if(gameState < -90) {
			...
			if(currTextTime == textTime) {
				currTextTime = 0;
				if(drawnLetters == menuText[menuState][drawnLines].replace('!!!!!!!!!!!!!', topScores[0][0] + ': ' + topScores[0][1]).replace('@@@@@@@@@@@@@', topScores[1][0] + ': ' + topScores[1][1]).replace('#############', topScores[2][0] + ': ' + topScores[2][1]).replace('$$$$$$$$$$$$$', topScores[3][0] + ': ' + topScores[3][1]).replace('%%%%%%%%%%%%%', topScores[4][0] + ': ' + topScores[4][1]).replace('^^^^^^^^^^^^^', score).replace('&&&&&&&&&&&&&', (rank+1).toString()).replace('*************', ((name.length == 0) ? '_' : name)).length) {
